...
| `chat` | someone posted in the meeting **chat** (`sender` + `text`) | **be conservative — most chat is links, asides, or banter.** Only `{"cmd":"record","for":"<sender>","text":"…"}` when it clearly reads as that person's standup update (or they said they'd post it, e.g. bad mic). A fix → `solution`. Everything else → `{"cmd":"none"}` — never pad the standup with chat noise |
