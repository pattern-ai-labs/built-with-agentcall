...
| `followup` | a cross-day follow-up answer (their `prior` blockers are listed). **Only fires when `TRACK_ACROSS_DAYS` is ON — it is OFF by default, so normally you never see this** | judge it: `{"cmd":"resolve","for":"<name>","cleared":true｜false}` |
